...
// ADR-063: explicit native-client access. Browsers always send an Origin and
// stay governed by the MOMENTUM_ALLOWED_ORIGINS allowlist. React Native fetch
// on a device sends no Origin header at all; such requests are admitted only
// when they carry this fixed client header with this exact value. The header
// is an explicit, documented identification of the approved native path — not
// a secret and not authentication. Public authentication remains a separate
// release blocker (Generator Service Runbook, production prerequisites).
export const NATIVE_CLIENT_HEADER = 'x-momentum-client';
export const NATIVE_CLIENT_VALUE = 'native';

export const parseAllowedOrigins = (env) =>
  new Set((env.MOMENTUM_ALLOWED_ORIGINS || 'http://127.0.0.1:8081,http://localhost:8081').split(',').map((origin) => origin.trim()));

// Access decision for POST /v1/experience-drafts:
// - Origin present  → the browser path; the origin must be allowlisted.
// - Origin absent   → the native path; the fixed client header must match.
// Node lowercases header names, so pass request.headers[NATIVE_CLIENT_HEADER].
export const isRequestAllowed = (origin, clientHeaderValue, allowedOrigins) => {
  if (origin) return allowedOrigins.has(origin);
  return clientHeaderValue === NATIVE_CLIENT_VALUE;
};

export const createGeneratorServer = (env = process.env, argv = process.argv) => {
  ...
  const provider = selectProvider(env, argv);
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === 'OPTIONS') {
      response.writeHead(204, {
        ...
        'access-control-allow-headers': `content-type,${NATIVE_CLIENT_HEADER}`,
  ...
  return { server, port, host, provider, resolveRuntime };
};

const isMain = process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href;

if (isMain) {
  const { server, port, host, provider, resolveRuntime } = createGeneratorServer();
  server.listen(port, host, () => {
    const { isModel, configured } = resolveRuntime();
    const mode = isModel ? `${provider.name}:${provider.modelName(process.env)}${configured ? '' : ' (not configured)'}` : provider.name;
