...
const birdOpportunity = (bird: BirdObservation, snapshot: LiveWorldSnapshot, context: PrototypeContext, maxTravelMinutes: number, bike: boolean): LivingWorldOpportunity | { id: string; reason: string } => {
  ...
  return {
    ...
    destinationName: bird.locationName, destination, experienceKind: 'outside', route, evidence: [evidence, ...(currentWeather ? [currentWeather] : [])],
...
export function composeLivingWorldOpportunities(snapshot: LiveWorldSnapshot, context: PrototypeContext, options: { maxTravelMinutes: number; bike: boolean }): OpportunityEngineResult {
  ...
  if (!Number.isFinite(age) || age > 2 * 3600000) return { ready: [], withheld: [{ id: 'snapshot', reason: 'De regionale bronset is te oud voor nieuwe kansen.' }], considered: 0, sourceLabel: 'Living World verlopen', sourceMix: [], perspectiveCount: 0 };
  const raw = [
  ...
  const sourceMix = [...new Set(ready.flatMap((item) => item.evidence.map((evidence) => evidence.sourceName)))];
  return { ready, withheld, considered: raw.length, sourceLabel: ready.length ? `${ready.length} uitvoerbare kansen uit actuele bronnen` : 'Geen uitvoerbare actuele kans', sourceMix, perspectiveCount: new Set(ready.map((item) => item.experienceKind)).size };
}
...
export function opportunityToExperience(opportunity: LivingWorldOpportunity): Experience {
  ...
  return {
    ...
    steps: [
      ...
      { title: route.arrivalPlan?.label ?? 'Beleef één detail volledig', instruction: route.arrivalPlan?.instruction ?? (opportunity.experienceKind === 'outside' ? 'Kijk breed naar landschap, licht, geluid en beweging. Laat één soort of plek nooit de hele ervaring bepalen.' : 'Kies één beeld, verhaal, smaak, ontmoeting of vraag. Je hoeft de plek niet af te werken.'), meta: route.arrivalPlan?.radiusMeters ? `rond het anker · tot circa ${route.arrivalPlan.radiusMeters} m` : `${route.experienceMinutes} min ter plaatse` },
    ...
    guideOrigin: { mode: 'composed', label: 'Opportunity Engine v3 · bronmix, perspectief en routegrens' },
    blueprint: { id: 'living-world-opportunity-v3', domain: opportunity.experienceKind, origin: 'deterministic', validationLabel: 'Bronvenster, openbare bestemming, buitencondities, tijdsbudget en aankomstplan gecontroleerd' },
  };
}
