...
// Legacy style keys still named “editorial” intentionally resolve to the same app-wide family.
const editorialFont = appFont;
...
function NowScreen({ firstName, suggestions, resumableExperience, context, calendar, liveWorld, liveLoading, generatorStatus, generatingMoment, onGenerateMoment, onResume, onDiscardSession, onOpen, onProfile, onDiscover, onFeedback }: { firstName: string; suggestions: Array<{ experience: Experience; decision: LocalDecision }>; resumableExperience?: Experience; context: PrototypeContext; calendar: CalendarContextSnapshot; liveWorld: LiveWorldSnapshot | null; liveLoading: boolean; generatorStatus: GeneratorRuntimeStatus; generatingMoment: boolean; onGenerateMoment: () => void; onResume: () => void; onDiscardSession: () => void; onOpen: (item: Experience, stage?: FlowStage) => void; onProfile: () => void; onDiscover: () => void; onFeedback: (item: Experience, outcome: LearningOutcome) => void }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenScroll} showsVerticalScrollIndicator={false}>
      ...
      {decision.confidence === 'low' && !declined ? <View style={styles.silentCard}>
        ...
        <View style={styles.heroCard}>
          <ImageBackground source={{ uri: experience.image }} style={styles.heroImage} imageStyle={styles.heroImageStyle}>
            ...
            <View style={styles.heroTop}><Pill label={`${experienceKindLabels[experience.kind].toUpperCase()} MOMENT`} accent={experience.accent} /><Text style={styles.heroTime}>{grounding.currentCount ? `LIVE VERRIJKT · ${grounding.currentCount}` : experience.generation ? 'VOOR DIT MOMENT GEMAAKT' : suggestionIndex === 0 ? 'BESTE MATCH' : 'ANDERE BLIK'}</Text></View>
            <View style={styles.heroBottom}>
              <Text style={styles.heroTitle}>{experience.title}</Text>
              <Text style={styles.heroPromise}>{experience.promise}</Text>
              {(experience.generation || experience.liveEvidence?.length) ? <View style={styles.heroGrounding}><Text style={styles.heroGroundingMark}>{grounding.currentCount ? '●' : '○'}</Text><Text style={styles.heroGroundingText}>{experience.generation ? 'Nieuw samengesteld' : 'Geselecteerd'} · {grounding.label}</Text></View> : null}
              <View style={styles.heroFacts}>
                <MiniFact value={`${experience.duration} min`} label={experienceFactLabels[experience.kind].duration} />
                <MiniFact value={experience.distance ?? (experience.kind === 'food' ? 'zelf kiezen' : 'dichtbij')} label={experienceFactLabels[experience.kind].distance} />
                <MiniFact value={experience.effort} label={experienceFactLabels[experience.kind].effort} />
              </View>
            </View>
          </ImageBackground>
      ...
      <View style={styles.momentMakerCard}>
        <View style={styles.momentMakerHeader}><View style={[styles.sourceState, generatorStatus.state === 'model' ? styles.sourceLive : generatorStatus.state === 'offline' ? styles.sourceError : styles.sourceWaiting]} /><Text style={styles.momentMakerEyebrow}>MOMENTMAKER · {generatorStatus.label.toUpperCase()}</Text></View>
        <Text style={styles.momentMakerTitle}>Zie hoe Momentum iets nieuws opbouwt.</Text>
        <Text style={styles.momentMakerBody}>{generatorStatus.detail} De uitkomst opent meteen als volledige capsule, zodat je belofte, voorbereiding, begeleiding en herinnering kunt beoordelen.</Text>
        <SecondaryButton label={generatingMoment ? 'Nieuw moment wordt gecontroleerd…' : 'Maak een nieuw moment voor nu'} onPress={onGenerateMoment} />
      </View>
              ...
              <View style={styles.timelineContent}>
                ...
                <ImageBackground source={{ uri: item.image }} style={styles.dayCardImage} imageStyle={styles.dayCardImageStyle}>
                  ...
                  <View style={styles.dayCardCopy}>
                    ...
                    {item.generation || item.liveEvidence?.length ? <Text style={styles.directionMatch}>{item.generation ? 'Nieuw samengesteld' : 'Contextueel gekozen'} · {evidenceSummary(item).label}</Text> : directionReason && <Text style={styles.directionMatch}>Past bij een richting die jij zelf benoemde</Text>}
                  </View>
                </ImageBackground>
              </View>
            </Pressable>
    ...
    <ScrollView contentContainerStyle={styles.flowScroll} showsVerticalScrollIndicator={false}>
      ...
      <ImageBackground source={{ uri: experience.image }} style={styles.prepareExpectationCard} imageStyle={styles.prepareExpectationImage}>
        ...
        <View style={styles.prepareExpectationCopy}>
          <Text style={styles.expectationLabel}>DIT WACHT OP JE</Text>
          <Text style={styles.expectationTitle}>{experience.wonder}</Text>
      ...
      <View style={styles.readySummary}>
        ...
        <Pressable accessibilityRole="button" accessibilityState={{ expanded: detailsOpen }} onPress={() => setDetailsOpen((value) => !value)} style={styles.adjustButton}><Text style={styles.adjustButtonText}>{detailsOpen ? 'Minder' : 'Pas aan'}</Text></Pressable>
      </View>
      {detailsOpen && <>
...
function PresenceScreen({ experience, personal, company, guideDepth, shared, initialStep, onStepChange, onBack, onFinish }: { experience: Experience; personal: PersonalProfile; company: Company; guideDepth: GuideDepth; shared?: SharedCapsuleState; initialStep: number; onStepChange: (stepIndex: number) => void; onBack: () => void; onFinish: () => void }) {
  ...
  return (
    <ImageBackground source={{ uri: experience.image }} style={styles.presenceScreen} imageStyle={styles.presenceBackdrop}>
      <View style={styles.presenceBackdropShade} />
...
function BottomNav({ surface, onChange }: { surface: Surface; onChange: (surface: Surface) => void }) {
  ...
  return <View style={styles.bottomNav}>{items.map((item) => <Pressable key={item.id} accessibilityRole="tab" accessibilityState={{ selected: surface === item.id }} onPress={() => onChange(item.id)} style={[styles.navItem, surface === item.id && styles.navItemActive]}><Text style={[styles.navIcon, surface === item.id && styles.navActive]}>{item.icon}</Text><Text style={[styles.navLabel, surface === item.id && styles.navActive]}>{item.label}</Text></Pressable>)}</View>;
}
...
function PrimaryButton({ label, onPress }: { label: string; onPress: () => void }) { return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} style={({ pressed }) => [styles.primaryButton, pressed && styles.pressed]}><Text style={styles.primaryButtonText}>{label}</Text><Text style={styles.primaryArrow}>→</Text></Pressable>; }
function SecondaryButton({ label, onPress }: { label: string; onPress: () => void }) { return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} style={({ pressed }) => [styles.secondaryButton, pressed && styles.pressed]}><Text style={styles.secondaryButtonText}>{label}</Text></Pressable>; }
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: colors.ink }, safe: { flex: 1, alignItems: 'center' }, appFrame: { flex: 1, width: '100%', maxWidth: 560 }, webAppFrame: { borderLeftWidth: 1, borderRightWidth: 1, borderColor: colors.softLine, backgroundColor: 'rgba(8,11,10,0.96)', shadowColor: '#000000', shadowOpacity: 0.42, shadowRadius: 42, shadowOffset: { width: 0, height: 0 } }, flex: { flex: 1 },
  ...
  onboardingTitle: { color: colors.bone, fontSize: 39, lineHeight: 44, letterSpacing: -1.1, fontWeight: '700', fontFamily: appFont, maxWidth: 430 },
  ...
  onboardingChoiceSelected: { borderColor: colors.green, backgroundColor: 'rgba(182,204,130,0.09)' },
  onboardingChoiceTitle: { color: colors.bone, fontSize: 15, fontWeight: '600' },
  onboardingFooter: { marginTop: 'auto', paddingTop: 24 },
  trustCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, borderLeftWidth: 3, borderLeftColor: colors.green, backgroundColor: colors.panel, padding: 17, marginTop: 8 },
  ...
  insightEyebrow: { color: colors.green, fontSize: 9, letterSpacing: 1.2, fontWeight: '700' },
  ...
  ambientGold: { position: 'absolute', width: 520, height: 520, borderRadius: 260, backgroundColor: 'rgba(215,183,123,0.08)', top: -350, right: -260 },
  ambientGreen: { position: 'absolute', width: 500, height: 500, borderRadius: 250, backgroundColor: 'rgba(182,204,130,0.07)', bottom: -330, left: -270 },
  ...
  heroCard: { borderRadius: radii.hero, overflow: 'hidden', backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.line }, heroImage: { height: 390, padding: 20, justifyContent: 'space-between' }, heroImageStyle: { borderTopLeftRadius: radii.hero - 1, borderTopRightRadius: radii.hero - 1 }, imageShade: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, backgroundColor: colors.scrim },
  heroTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, heroTime: { color: colors.bone, fontSize: 12, letterSpacing: 1.2, fontWeight: '600' }, heroBottom: { gap: 11 }, heroTitle: { color: colors.bone, fontSize: 37, lineHeight: 42, fontWeight: '700', fontFamily: appFont, letterSpacing: -1 }, heroPromise: { color: 'rgba(243,240,233,0.88)', fontSize: 16, lineHeight: 23, maxWidth: 390 }, heroGrounding: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 7, borderRadius: radii.pill, borderWidth: 1, borderColor: 'rgba(182,204,130,0.35)', backgroundColor: 'rgba(5,7,6,0.58)', paddingHorizontal: 11, paddingVertical: 7 }, heroGroundingMark: { color: colors.green, fontSize: 10 }, heroGroundingText: { color: colors.bone, fontSize: 11, fontWeight: '700' }, heroFacts: { flexDirection: 'row', gap: 18, marginTop: 8 },
  ...
  liveWorldBar: { minHeight: 62, borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, paddingHorizontal: 15, flexDirection: 'row', alignItems: 'center', gap: 11, marginBottom: 14 }, liveWorldBarTitle: { color: colors.bone, fontSize: 12, fontWeight: '600' }, liveWorldBarDetail: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 3 }, liveWorldMark: { color: colors.green, fontSize: 20 },
  ...
  miniFact: { minWidth: 70 }, miniFactValue: { color: colors.bone, fontSize: 13, fontWeight: '600' }, miniFactLabel: { color: 'rgba(244,238,227,0.68)', fontSize: 11, marginTop: 3 },
  primaryButton: { minHeight: 56, borderRadius: radii.control, backgroundColor: colors.green, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, primaryButtonText: { color: colors.ink, fontSize: 15, fontWeight: '700' }, primaryArrow: { color: colors.ink, fontSize: 22 },
  ...
  momentMakerCard: { marginTop: 18, borderRadius: 25, borderWidth: 1, borderColor: 'rgba(164,197,93,0.3)', backgroundColor: 'rgba(164,197,93,0.055)', padding: 17 }, momentMakerHeader: { flexDirection: 'row', alignItems: 'center', gap: 9, marginBottom: 11 }, momentMakerEyebrow: { color: colors.green, fontSize: 9, letterSpacing: 1.2, fontWeight: '700' }, momentMakerTitle: { color: colors.bone, fontSize: 19, lineHeight: 24, fontWeight: '600' }, momentMakerBody: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 7, marginBottom: 13 },
  ...
  daySummary: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 17, marginBottom: 26 }, daySummaryTitle: { color: colors.bone, fontSize: 17, fontWeight: '700' }, daySummaryBody: { color: colors.muted, fontSize: 12, lineHeight: 18, marginTop: 5 }, timeline: { gap: 0 }, timelineRow: { flexDirection: 'row' }, timelineRail: { width: 28, alignItems: 'center' }, timelineDot: { width: 8, height: 8, borderRadius: 4, marginTop: 4 }, timelineLine: { flex: 1, width: 1, backgroundColor: colors.line, marginVertical: 8 }, timelineContent: { flex: 1, paddingBottom: 22 }, timelineTime: { color: colors.gold, fontSize: 10, letterSpacing: 1.15, fontWeight: '700', marginBottom: 9 }, dayCardImage: { minHeight: 186, justifyContent: 'flex-end' }, dayCardImageStyle: { borderRadius: radii.card }, dayCardCopy: { padding: 17 }, dayCardTitle: { color: colors.bone, fontSize: 24, lineHeight: 29, fontWeight: '700', fontFamily: appFont }, dayCardPromise: { color: 'rgba(243,240,233,0.82)', fontSize: 13, lineHeight: 19, marginTop: 6, maxWidth: 330 }, dayCardMeta: { color: colors.bone, fontSize: 11, lineHeight: 16, fontWeight: '600', marginTop: 12 }, quietDay: { borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 17, marginTop: 2 }, quietDayTitle: { color: colors.bone, fontSize: 15, fontWeight: '700' }, quietDayBody: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 5 },
  ...
  intentPanel: { borderRadius: radii.hero, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 20 }, fieldLabel: { color: colors.green, fontSize: 10, letterSpacing: 1.3, fontWeight: '700', marginTop: 8, marginBottom: 12 }, chipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 24 }, choiceChip: { minHeight: 44, borderRadius: 99, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panelRaised, paddingHorizontal: 15, paddingVertical: 11, justifyContent: 'center' }, choiceChipSelected: { borderColor: colors.green, backgroundColor: 'rgba(182,204,130,0.10)' }, choiceChipText: { color: colors.muted, fontSize: 13, fontWeight: '600' }, choiceChipTextSelected: { color: colors.bone }, intentInput: { minHeight: 118, borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.ink, color: colors.bone, fontSize: 16, lineHeight: 23, padding: 16, textAlignVertical: 'top', marginBottom: 14 }, orRow: { flexDirection: 'row', alignItems: 'center', marginTop: 13 }, orLine: { flex: 1, height: 1, backgroundColor: colors.line }, orText: { color: colors.muted, fontSize: 10, fontWeight: '600', marginHorizontal: 12 }, intentPrivacy: { color: colors.muted, fontSize: 10, lineHeight: 16, textAlign: 'center', marginTop: 10 }, interpretation: { borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 15, marginBottom: 24 }, interpretationLabel: { color: colors.green, fontSize: 10, letterSpacing: 1.25, fontWeight: '700' }, interpretationText: { color: colors.bone, fontSize: 13, lineHeight: 19, marginTop: 7 }, sectionLabel: { color: colors.green, fontSize: 10, letterSpacing: 1.3, fontWeight: '700', marginTop: 8, marginBottom: 10 }, experienceTile: { marginBottom: 20 }, tileImage: { minHeight: 210, justifyContent: 'flex-end' }, tileImageLarge: { minHeight: 300 }, tileImageStyle: { borderRadius: radii.card }, generatedTileBadge: { position: 'absolute', right: 14, top: 14, borderRadius: 99, borderWidth: 1, borderColor: colors.line, backgroundColor: 'rgba(11,14,13,0.82)', paddingHorizontal: 11, paddingVertical: 8 }, generatedTileBadgeText: { color: colors.green, fontSize: 8, letterSpacing: 1, fontWeight: '700' }, tileCopy: { padding: 18 }, tileTitle: { color: colors.bone, fontSize: 27, lineHeight: 32, fontWeight: '700', fontFamily: appFont, letterSpacing: -0.5, marginTop: 12 }, tilePromise: { color: 'rgba(243,240,233,0.84)', fontSize: 13, lineHeight: 19, marginTop: 7 }, tileMeta: { color: colors.bone, fontSize: 11, lineHeight: 16, fontWeight: '600', marginTop: 14 }, generatedJourneyCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, borderLeftWidth: 3, borderLeftColor: colors.green, backgroundColor: colors.panel, padding: 16, marginTop: -7, marginBottom: 20 }, generatedJourneyHeader: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }, generatedJourneyEyebrow: { color: colors.green, fontSize: 9, letterSpacing: 1.25, fontWeight: '700' }, generatedJourneyTitle: { color: colors.bone, fontSize: 17, fontWeight: '700', marginTop: 5 }, generatedJourneyCount: { color: colors.muted, fontSize: 10, marginTop: 2 }, generatedJourneyStages: { marginTop: 14 }, generatedJourneyStage: { minHeight: 62, borderTopWidth: 1, borderTopColor: colors.line, flexDirection: 'row', alignItems: 'center', paddingVertical: 10 }, generatedJourneyNumber: { width: 28, height: 28, borderRadius: 14, borderWidth: 1, alignItems: 'center', justifyContent: 'center', marginRight: 11 }, generatedJourneyNumberText: { color: colors.bone, fontSize: 10 }, generatedJourneyStageTitle: { color: colors.bone, fontSize: 12, fontWeight: '700' }, generatedJourneyStageBody: { color: colors.muted, fontSize: 9, lineHeight: 14, marginTop: 3 }, generatedJourneyCoverage: { color: colors.muted, fontSize: 9, lineHeight: 14, borderTopWidth: 1, borderTopColor: colors.line, paddingTop: 11 }, finiteNote: { color: colors.muted, fontSize: 10, lineHeight: 15, textAlign: 'center', marginTop: 2 },
  clarificationPanel: { borderRadius: radii.hero, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 20 }, clarificationTitle: { color: colors.bone, fontFamily: appFont, fontSize: 30, lineHeight: 36, fontWeight: '700', marginTop: 13 }, clarificationBody: { color: colors.muted, fontSize: 14, lineHeight: 21, marginTop: 10 }, intentQuote: { color: colors.gold, fontFamily: appFont, fontSize: 16, lineHeight: 23, fontWeight: '600', marginTop: 12 }, clarificationOptions: { gap: 9, marginTop: 24, marginBottom: 18 }, clarificationOption: { minHeight: 58, borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panelRaised, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center' }, clarificationOptionText: { color: colors.bone, fontSize: 15, fontWeight: '600', flex: 1 },
  ...
  lifeSummary: { flexDirection: 'row', alignItems: 'center', borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 17, marginBottom: 26 }, lifeSummaryBig: { color: colors.green, fontSize: 44, fontWeight: '300', marginRight: 16 }, lifeSummaryTitle: { color: colors.bone, fontSize: 15, fontWeight: '700' }, lifeSummaryBody: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 4 }, memoryGrid: { gap: 14 }, memoryCard: { minHeight: 224 }, memoryImage: { minHeight: 224, justifyContent: 'flex-end' }, memoryImageStyle: { borderRadius: radii.card }, memoryCopy: { padding: 18 }, memoryDate: { color: colors.gold, fontSize: 10, letterSpacing: 1.1, fontWeight: '700' }, memoryTitle: { color: colors.bone, fontFamily: appFont, fontSize: 26, lineHeight: 31, fontWeight: '700', letterSpacing: -0.4, marginTop: 7 }, memoryNote: { color: 'rgba(243,240,233,0.8)', fontSize: 12, lineHeight: 18, marginTop: 6 }, memoryMeaning: { color: colors.green, fontSize: 10, lineHeight: 15, fontWeight: '600', marginTop: 9 }, memoryShared: { color: colors.green, fontSize: 10, lineHeight: 15, marginTop: 8 }, learningCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, borderLeftWidth: 3, borderLeftColor: colors.green, backgroundColor: colors.panel, padding: 17, marginTop: 22 }, learningTitle: { color: colors.green, fontSize: 13, fontWeight: '700' }, learningBody: { color: colors.muted, fontSize: 11, lineHeight: 18, marginTop: 7 },
  bottomNav: { position: 'absolute', left: 14, right: 14, bottom: 10, minHeight: 70, borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, backgroundColor: 'rgba(20,24,22,0.97)', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 7 }, navItem: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 4, minHeight: 56, borderRadius: radii.control }, navItemActive: { backgroundColor: 'rgba(182,204,130,0.09)' }, navIcon: { color: colors.muted, fontSize: 16 }, navLabel: { color: colors.muted, fontSize: 10, fontWeight: '600' }, navActive: { color: colors.green },
  backButton: { alignSelf: 'flex-start', minHeight: 44, justifyContent: 'center', marginBottom: 10 }, backButtonText: { color: colors.muted, fontSize: 13, fontWeight: '600' }, detailHero: { height: 390, justifyContent: 'flex-end', marginHorizontal: -20, marginTop: -62, marginBottom: 26 }, detailHeroImage: { borderBottomLeftRadius: radii.hero, borderBottomRightRadius: radii.hero }, detailHeroCopy: { padding: 22, paddingTop: 112 }, detailTitle: { color: colors.bone, fontSize: 38, lineHeight: 43, fontWeight: '700', fontFamily: appFont, letterSpacing: -1, marginTop: 13 }, detailPromise: { color: 'rgba(243,240,233,0.86)', fontSize: 15, lineHeight: 22, marginTop: 10 }, wonderHeadline: { color: colors.green, fontSize: 10, letterSpacing: 1.35, fontWeight: '700' }, wonderLarge: { color: colors.bone, fontSize: 20, lineHeight: 28, fontWeight: '600', fontFamily: appFont, marginTop: 9 }, factStrip: { flexDirection: 'row', gap: 14, borderWidth: 1, borderColor: colors.line, borderRadius: radii.control, backgroundColor: colors.panel, paddingHorizontal: 14, paddingVertical: 15, marginVertical: 20 },
  liveEvidenceCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, borderLeftWidth: 3, borderLeftColor: colors.green, backgroundColor: colors.panel, padding: 16, gap: 12, marginBottom: 20 }, liveEvidenceTitle: { color: colors.green, fontSize: 11, letterSpacing: 1.35, fontWeight: '700' }, liveEvidenceRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 }, liveEvidenceDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: colors.green, marginTop: 5 }, liveEvidenceLabel: { color: colors.bone, fontSize: 12, lineHeight: 17, fontWeight: '600' }, liveEvidenceMeta: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 3 }, liveEvidenceCaution: { color: colors.muted, fontSize: 10, lineHeight: 15, borderTopWidth: 1, borderTopColor: colors.line, paddingTop: 10 }, expiredEvidenceCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 15, marginBottom: 20 }, expiredEvidenceText: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 7 },
  blueprintBadge: { minHeight: 64, borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 13, marginTop: -6, marginBottom: 20, flexDirection: 'row', alignItems: 'center', gap: 11 }, blueprintBadgeMark: { color: colors.green, fontSize: 18 }, blueprintBadgeTitle: { color: colors.bone, fontSize: 13, fontWeight: '700' }, blueprintBadgeBody: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 3 },
  meaningThreadCard: { borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 16, marginBottom: 20 }, meaningThreadCompact: { marginTop: 14, marginBottom: 2 }, meaningThreadEyebrow: { color: colors.green, fontSize: 11, letterSpacing: 1.25, fontWeight: '700' }, meaningThreadLabel: { color: colors.bone, fontFamily: appFont, fontSize: 18, lineHeight: 24, fontWeight: '600', marginTop: 8 }, meaningThreadReason: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 7 },
  flowTitle: { color: colors.bone, fontSize: 38, lineHeight: 43, fontWeight: '700', fontFamily: appFont, letterSpacing: -1 },
  expectationCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 18, marginTop: 22 }, expectationLabel: { color: colors.green, fontSize: 11, letterSpacing: 1.45, fontWeight: '700' }, expectationTitle: { color: colors.bone, fontSize: 22, lineHeight: 29, fontWeight: '600', marginTop: 9 }, expectationBody: { color: colors.muted, fontSize: 13, lineHeight: 20, marginTop: 8 },
  ...
  prepareExpectationShade: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, backgroundColor: 'rgba(5,8,7,0.48)' },
  prepareExpectationCopy: { padding: 20, paddingTop: 92, backgroundColor: 'rgba(7,10,9,0.30)' },
  prepareExpectationBody: { color: 'rgba(246,242,234,0.88)', fontSize: 15, lineHeight: 22, marginTop: 9, maxWidth: 410 },
  prepareLiveCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, borderLeftWidth: 3, borderLeftColor: colors.green, backgroundColor: colors.panel, padding: 16, gap: 12, marginTop: 14, marginBottom: 22 }, prepareLiveRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 },
  editorialDepthCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 16, marginTop: 14, marginBottom: 22 }, editorialDepthText: { color: colors.bone, fontSize: 16, lineHeight: 23, fontWeight: '600', marginTop: 8 }, editorialDepthSource: { color: colors.muted, fontSize: 9, lineHeight: 14, marginTop: 8 },
  ...
  presenceBackdropShade: { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0, backgroundColor: 'rgba(5,8,7,0.74)' },
  presenceTopRow: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', zIndex: 2 }, presenceActions: { flexDirection: 'row', alignItems: 'center', gap: 8 }, guideButton: { minHeight: 44, paddingHorizontal: 15, borderRadius: 22, borderWidth: 1, borderColor: 'rgba(246,242,234,0.16)', backgroundColor: 'rgba(10,14,12,0.70)', alignItems: 'center', justifyContent: 'center' }, guideButtonText: { color: colors.bone, fontSize: 12, fontWeight: '700' }, phoneAwayButton: { minHeight: 44, paddingHorizontal: 15, borderRadius: 22, borderWidth: 1, borderColor: 'rgba(185,207,119,0.46)', backgroundColor: 'rgba(10,14,12,0.72)', alignItems: 'center', justifyContent: 'center' }, phoneAwayButtonText: { color: colors.green, fontSize: 12, fontWeight: '700' },
  presenceStage: { width: '100%', maxWidth: 470, alignItems: 'center', borderRadius: 30, borderWidth: 1, borderColor: 'rgba(246,242,234,0.12)', backgroundColor: 'rgba(10,14,12,0.80)', paddingHorizontal: 22, paddingVertical: 28, shadowColor: '#000000', shadowOpacity: 0.34, shadowRadius: 28, shadowOffset: { width: 0, height: 14 } },
  presenceTitle: { color: colors.bone, fontSize: 38, lineHeight: 43, fontWeight: '700', fontFamily: appFont, textAlign: 'center', letterSpacing: -0.7 }, presenceCue: { color: 'rgba(246,242,234,0.76)', fontSize: 16, lineHeight: 24, textAlign: 'center', maxWidth: 390, marginTop: 16 }, presenceUnit: { color: colors.muted, fontSize: 11, letterSpacing: 1.15, fontWeight: '600' }, presenceFooter: { color: colors.muted, fontSize: 11, lineHeight: 16, textAlign: 'center', marginTop: 9 },
  presenceFooterPanel: { paddingTop: 12, borderTopWidth: 1, borderTopColor: 'rgba(246,242,234,0.08)' },
  phoneAwayScreen: { flex: 1, padding: 28, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.ink }, phoneAwayEyebrow: { color: colors.green, fontSize: 9, letterSpacing: 2.2, fontWeight: '700' }, phoneAwayTitle: { color: colors.bone, fontSize: 28, lineHeight: 34, fontWeight: '700', fontFamily: appFont, textAlign: 'center', marginTop: 14 }, phoneAwayCue: { color: colors.muted, fontSize: 15, textAlign: 'center', marginTop: 14 }, phoneAwayTimer: { color: colors.bone, fontSize: 62, fontWeight: '200', fontVariant: ['tabular-nums'], marginTop: 34 }, phoneAwayOrb: { width: 130, height: 130, borderRadius: 65, borderWidth: 2, alignItems: 'center', justifyContent: 'center', marginTop: 34 }, phoneAwayOrbText: { fontSize: 48 }, phoneAwayTogether: { color: colors.green, fontSize: 11, marginTop: 20 }, phoneAwayBody: { color: colors.muted, fontSize: 11, lineHeight: 17, textAlign: 'center', maxWidth: 300, marginTop: 26 }, reopenGuide: { minHeight: 48, borderRadius: 24, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, paddingHorizontal: 22, alignItems: 'center', justifyContent: 'center', marginTop: 26 }, reopenGuideText: { color: colors.bone, fontSize: 13, fontWeight: '700' },
  ...
  memoryPreview: { minHeight: 268, justifyContent: 'flex-end', marginVertical: 26 }, memoryPreviewTitle: { color: colors.bone, fontFamily: appFont, fontSize: 29, lineHeight: 34, fontWeight: '700', letterSpacing: -0.5, padding: 18 }, sharedMemoryCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, borderLeftWidth: 3, borderLeftColor: colors.green, backgroundColor: colors.panel, padding: 16, marginBottom: 16 }, sharedMemoryTitle: { color: colors.bone, fontSize: 17, fontWeight: '700', marginTop: 8 }, sharedMemoryBody: { color: colors.muted, fontSize: 10, lineHeight: 16, marginTop: 5 }, meaningTraceCard: { borderRadius: radii.card, borderWidth: 1, borderColor: colors.line, backgroundColor: colors.panel, padding: 16, marginBottom: 16 }, meaningTraceTitle: { color: colors.bone, fontSize: 15, lineHeight: 21, fontWeight: '600', marginTop: 8 }, meaningTraceBody: { color: colors.muted, fontSize: 10, lineHeight: 15, marginTop: 7 }, memoryInput: { minHeight: 116, borderRadius: radii.control, borderWidth: 1, borderColor: colors.line, color: colors.bone, fontSize: 16, lineHeight: 23, padding: 16, textAlignVertical: 'top', marginBottom: 16, backgroundColor: colors.panel },
