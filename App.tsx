        ...
        <View style={[styles.appFrame, Platform.OS === 'web' && styles.webAppFrame]}>
          {flowStage === null && (
            <>
              ...
              {surface === 'lifebook' && <LifeBookScreen memories={memories} onProfile={() => setFlowStage('profile')} onOpen={(item) => { setPersonalProfile((current) => applyLearning(current, item, 'repeat')); openExperience(item, 'lifebook'); }} />}
...
function NowScreen({ firstName, suggestions, resumableExperience, context, calendar, liveWorld, liveLoading, generatorStatus, generatingMoment, onGenerateMoment, onResume, onDiscardSession, onOpen, onProfile, onDiscover, onFeedback }: { firstName: string; suggestions: Array<{ experience: Experience; decision: LocalDecision }>; resumableExperience?: Experience; context: PrototypeContext; calendar: CalendarContextSnapshot; liveWorld: LiveWorldSnapshot | null; liveLoading: boolean; generatorStatus: GeneratorRuntimeStatus; generatingMoment: boolean; onGenerateMoment: () => void; onResume: () => void; onDiscardSession: () => void; onOpen: (item: Experience, stage?: FlowStage) => void; onProfile: () => void; onDiscover: () => void; onFeedback: (item: Experience, outcome: LearningOutcome) => void }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.screenScroll} showsVerticalScrollIndicator={false}>
      <ScreenHeader eyebrow={`${dayPartLabels[context.dayPart].toUpperCase()}${firstName ? ` · ${firstName.toUpperCase()}` : ''}`} title="Vandaag wacht er iets moois op je." subtitle="Dit past waarschijnlijk bij je moment. Jij beslist." onProfile={onProfile} />
...
function RememberScreen({ experience, personal, shared, onSkip, onSave }: { experience: Experience; personal: PersonalProfile; shared?: SharedCapsuleState; onSkip: () => void; onSave: (input: ReflectionInput, generationEvaluation?: GenerationEvaluationSignal[]) => void }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.flowScroll} keyboardShouldPersistTaps="handled">
      ...
      <ImageBackground source={{ uri: experience.image }} style={styles.memoryPreview} imageStyle={styles.memoryImageStyle}><ImageShade /><Text style={styles.memoryPreviewTitle}>{experience.title}</Text></ImageBackground>
...
function BottomNav({ surface, onChange }: { surface: Surface; onChange: (surface: Surface) => void }) {
  ...
  return <View style={styles.bottomNav}>{items.map((item) => {
    ...
    return <Pressable key={item.id} accessibilityRole="tab" accessibilityLabel={item.label} accessibilityState={{ selected: active }} onPress={() => onChange(item.id)} style={styles.navItem}><View style={[styles.navIconShell, active && styles.navIconShellActive]}><NavGlyph kind={item.id} active={active} /></View><Text style={[styles.navLabel, active && styles.navActive]}>{item.label}</Text></Pressable>;
  })}</View>;
}

function NavGlyph({ kind, active }: { kind: Surface; active: boolean }) {
  const color = active ? colors.green : colors.muted;
  if (kind === 'now') return <View style={[styles.glyphRing, { borderColor: color }]}><View style={[styles.glyphDot, { backgroundColor: color }]} /></View>;
  if (kind === 'today') return <View style={[styles.glyphSun, { borderColor: color }]}><View style={[styles.glyphSunLine, { backgroundColor: color, transform: [{ rotate: '45deg' }] }]} /><View style={[styles.glyphSunLine, { backgroundColor: color, transform: [{ rotate: '-45deg' }] }]} /></View>;
  if (kind === 'discover') return <View style={[styles.glyphCompass, { borderColor: color }]}><View style={[styles.glyphCompassNeedle, { borderBottomColor: color }]} /></View>;
  return <View style={styles.glyphBook}><View style={[styles.glyphBookPage, { borderColor: color }]} /><View style={[styles.glyphBookPage, { borderColor: color }]} /></View>;
}

function ImageShade() { return <><View style={styles.imageShade} /><View style={styles.imageBottomShade} /></>; }
...
function PrimaryButton({ label, onPress, disabled = false }: { label: string; onPress: () => void; disabled?: boolean }) { return <Pressable disabled={disabled} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled }} onPress={onPress} style={({ pressed }) => [styles.primaryButton, disabled && styles.primaryButtonDisabled, pressed && !disabled && styles.pressed]}><Text style={[styles.primaryButtonText, disabled && styles.primaryButtonTextDisabled]}>{label}</Text>{!disabled && <Text style={styles.primaryArrow}>→</Text>}</Pressable>; }
...
const styles = StyleSheet.create({
  ...
  resumeLabel: { color: colors.green, fontSize: 11, letterSpacing: 1, fontWeight: '700' },
  ...
  liveWorldBar: { minHeight: 54, paddingHorizontal: 4, flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 14 }, liveWorldBarTitle: { color: colors.bone, fontSize: 12, fontWeight: '600' }, liveWorldBarDetail: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 2 }, liveWorldMark: { color: colors.green, fontSize: 20 },
  ...
  primaryButton: { minHeight: 56, borderRadius: radii.control, backgroundColor: colors.green, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', shadowColor: colors.shadow, shadowOpacity: 0.12, shadowRadius: 14, shadowOffset: { width: 0, height: 6 } }, primaryButtonDisabled: { backgroundColor: colors.panelRaised, shadowOpacity: 0 }, primaryButtonText: { color: colors.onImage, fontSize: 15, fontWeight: '700' }, primaryButtonTextDisabled: { color: colors.muted }, primaryArrow: { color: colors.onImage, fontSize: 22 },
  ...
  whyButton: { minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 4 }, whyButtonText: { color: colors.muted, fontSize: 13 }, whyChevron: { color: colors.gold, fontSize: 18 }, whyPanel: { borderRadius: 18, backgroundColor: colors.accentSoft, padding: 15, gap: 8 }, whyReason: { color: colors.bone, fontSize: 12, lineHeight: 18 }, proofNote: { color: colors.muted, fontSize: 11, lineHeight: 16, marginTop: 3 }, feedbackActions: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }, feedbackDivider: { width: 1, height: 18, backgroundColor: colors.line, marginHorizontal: 8 }, quietAction: { minHeight: 44, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 10 }, quietActionText: { color: colors.muted, fontSize: 12 },
  ...
  lifeSummary: { flexDirection: 'row', alignItems: 'baseline', paddingBottom: 18, marginBottom: 8, borderBottomWidth: 1, borderBottomColor: colors.softLine }, lifeSummaryBig: { color: colors.green, fontSize: 32, fontWeight: '400', marginRight: 10 }, lifeSummaryTitle: { color: colors.muted, fontSize: 12, lineHeight: 18, flex: 1 }, memoryGrid: { gap: 12 }, memoryCard: { minHeight: 246 }, memoryCardCompact: { minHeight: 176 }, memoryImage: { minHeight: 246, justifyContent: 'flex-end' }, memoryImageCompact: { minHeight: 176 }, memoryImageStyle: { borderRadius: radii.card }, memoryCopy: { padding: 18 }, memoryDate: { color: colors.onImageAccent, fontSize: 11, letterSpacing: 1.05, fontWeight: '700' }, memoryTitle: { color: colors.onImage, fontFamily: appFont, fontSize: 27, lineHeight: 32, fontWeight: '700', letterSpacing: -0.4, marginTop: 7 }, memoryTitleCompact: { fontSize: 22, lineHeight: 27 }, memoryNote: { color: colors.onImageMuted, fontSize: 12, lineHeight: 18, marginTop: 6 }, memoryMeaning: { color: colors.onImageAccent, fontSize: 11, lineHeight: 16, fontWeight: '600', marginTop: 9 }, memoryShared: { color: colors.onImageAccent, fontSize: 11, lineHeight: 16, marginTop: 8 }, learningCard: { padding: 18, marginTop: 22, borderTopWidth: 1, borderTopColor: colors.softLine }, learningTitle: { color: colors.green, fontSize: 13, fontWeight: '700' }, learningBody: { color: colors.muted, fontSize: 12, lineHeight: 19, marginTop: 7 }, learningAction: { minHeight: 44, justifyContent: 'center', alignSelf: 'flex-start', marginTop: 8 }, learningActionText: { color: colors.green, fontSize: 12, fontWeight: '700' },
  bottomNav: { position: 'absolute', left: 18, right: 18, bottom: 12, minHeight: 70, borderRadius: radii.card, borderWidth: 1, borderColor: colors.softLine, backgroundColor: 'rgba(252,250,245,0.96)', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 5, shadowColor: colors.shadow, shadowOpacity: 0.10, shadowRadius: 20, shadowOffset: { width: 0, height: 9 } }, navItem: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 2, minHeight: 56, borderRadius: radii.control, ...(Platform.OS === 'web' ? ({ outlineStyle: 'none' } as any) : {}) }, navIconShell: { width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center' }, navIconShellActive: { backgroundColor: colors.accentSoft }, navLabel: { color: colors.muted, fontSize: 11, fontWeight: '600' }, navActive: { color: colors.green }, glyphRing: { width: 17, height: 17, borderRadius: 9, borderWidth: 1.7, alignItems: 'center', justifyContent: 'center' }, glyphDot: { width: 5, height: 5, borderRadius: 3 }, glyphSun: { width: 14, height: 14, borderRadius: 7, borderWidth: 1.7, alignItems: 'center', justifyContent: 'center' }, glyphSunLine: { position: 'absolute', width: 22, height: 1 }, glyphCompass: { width: 18, height: 18, borderRadius: 9, borderWidth: 1.5, alignItems: 'center', justifyContent: 'center', transform: [{ rotate: '18deg' }] }, glyphCompassNeedle: { width: 0, height: 0, borderLeftWidth: 3, borderRightWidth: 3, borderBottomWidth: 9, borderLeftColor: 'transparent', borderRightColor: 'transparent', transform: [{ rotate: '28deg' }] }, glyphBook: { width: 20, height: 17, flexDirection: 'row' }, glyphBookPage: { flex: 1, borderWidth: 1.4, borderTopLeftRadius: 4, borderTopRightRadius: 4 },
