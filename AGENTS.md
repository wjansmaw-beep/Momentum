...
The current application-code boundary is recorded in `00_Project/Status.md`. Only the Founder can extend that boundary, through an ADR.
...
The AI coding agent must explicitly select the appropriate working role for each substantive task. The Product Guardian role is always performed first; implementation begins only after that role has confirmed alignment or exposed the mismatch to be corrected.
...
Momentum is developed with interchangeable AI coding agents (for example Claude Code, Kimi, Codex, or Cursor). The repository governance — the Constitution, ADRs, and the required reading list — is what makes switching tools safe. Regardless of tool, every AI coding agent must:

- write no code outside the ADR boundary recorded in `00_Project/Status.md`;
- formulate any new scope as an ADR proposal for the Founder, never as direct implementation;
